...
> Design สุดท้ายหลังจาก synthesis จาก knowledge base v5, Codex feedback, และ iteration ระหว่างสองฝั่ง
> วันที่: 2026-05-05
...
| Language         | Python V1-V5       | Rust/advanced V6 ถ้าจำเป็น       |
...
✅ Benchmark: Sharpe > buy-and-hold BTC
✅ regime_performance.csv แสดงว่า strategy มี PF > 1.2
...
Components เพิ่ม:
  PaperBroker (live price + simulated fills)
    ...
    Latency model: decision(5ms) + network(50ms) + ack(20ms)
  ...
  Telegram alerts + remote commands
    /status /positions /pnl /pause_entries /resume_entries /halt
...
Min duration: 60 วัน paper หรือ 50+ trades (whichever is larger)

HARD GATES (zero tolerance):
  Unexpected open position = 0
...
**Regime role ใน V3-V4:** risk scaler

```
SIDEWAYS_LOW_VOL    → grid size ปกติ
SIDEWAYS_HIGH_VOL   → grid size -30%
UPTREND_LOW_VOL     → trend size ปกติ
UPTREND_HIGH_VOL    → trend size -30%
DOWNTREND_*         → ลด exposure, ไม่เปิด long ใหม่
BREAKOUT_WATCH      → ระวัง grid range หลุด
NO_TRADE            → pause entries ทุก strategy
```
...
Components เพิ่ม:
  ...
  DSR/PBO full implementation (V1.5 roadmap)
```
...
ดูรายละเอียดใน V2 section ด้านบน
...
def detect_regime(adx, ema_fast, ema_slow,
    ...
    # Hard filters
    if spread > config.max_spread:        return "NO_TRADE"
    ...
    is_uptrend   = adx > config.adx_trend and ema_fast > ema_slow
    is_downtrend = adx > config.adx_trend and ema_fast < ema_slow
    is_sideways  = adx < config.adx_sideways
