...
## 29. Review of Claude Research Set V3

### คำถาม

รวบรวมความรู้จาก Claude v3 แล้วหาองค์ความรู้ที่น่าสนใจและเป็นประโยชน์ โดยถ้าเจอคำแนะนำอย่าเพิ่งเชื่อทันที

### คำตอบสรุป

v3 เพิ่มของที่มีประโยชน์ 3 กลุ่มหลัก:

- Validation Methodology & Bias Controls
- Drift Detection & Action Tiers
- Framework Selection & Spike Plan

ควรรับเข้า:

- Validation pipeline 8 ขั้นตอน:
  - Unit/Logic
  - Bias Checks
  - In-sample
  - Robustness
...
- Survivorship bias policy โดยเฉพาะ V1 เริ่ม BTC/ETH ก่อน
- Drift Detection 4 ชนิด: Data, Execution, Concept, Performance
- ลำดับตรวจ drift: Data -> Execution -> Performance -> Concept
- Action tiers: Watch, Warning, Critical, Emergency, Retire
- Framework Spikes A-D ที่ใช้ strategy/data เดียวกันเพื่อเทียบ platform

ควร research เพิ่ม:

- DSR / PBO / CSCV implementation
- Drift threshold calibration
- Framework spike plan แบบปฏิบัติจริง
- Order Type -> Signal Urgency Mapping
- Paper Broker Implementation Details

ควรระวัง:

- claim ว่า 73% ของ automated crypto accounts ล้มเหลวใน 6 เดือนเพราะไม่มี drift monitoring ต้อง verify
- ตัวอย่าง survivorship bias top-20 altcoin ต้องมี source ก่อนใช้
- DSR/PBO thresholds เช่น DSR > 0 หรือ PBO < 20/30% ควรเป็น heuristic
- Freqtrade default หลัง spike ยังเร็วไป
- baseline first 90 วันของ live อาจ noise สูงถ้า trade น้อย

---

## 30. DSR / PBO / CSCV Implementation

### คำถาม

Research DSR / PBO / CSCV implementation ต่อ

### คำตอบสรุป

DSR/PBO/CSCV เป็นเครื่องมือกันการเลือกผู้ชนะจากการลองเยอะเกินไป ไม่ใช่เครื่องมือพิสูจน์ว่า strategy จะกำไรแน่นอน
...
Deflated Sharpe Ratio ใช้แก้ raw Sharpe จาก:

- multiple testing / selection bias
- non-normal returns เช่น skew/fat tails
- sample length สั้น
- dispersion ของ Sharpe จาก trials ทั้งหมด

Inputs:

- selected_returns
- observed_sharpe
- all_variant_sharpes หรือ number_of_trials
- number_of_observations
- skewness
- kurtosis
- effective_number_of_trials
- correlation ระหว่าง trials ถ้าประเมินได้

### PBO / CSCV

CSCV ขั้นตอน:

1. แบ่ง timeline เป็น folds เท่า ๆ กัน
2. เลือกครึ่งหนึ่งเป็น in-sample อีกครึ่งเป็น out-of-sample
3. เลือก variant ที่ดีที่สุดใน in-sample
4. ดู rank ของ variant นั้นใน out-of-sample
5. แปลง rank เป็น logit
6. PBO = สัดส่วน split ที่ OOS rank แย่กว่าค่ากลาง

Threshold เป็น heuristic:

```text
DSR / PSR:
  > 0.95 strong evidence
  0.80-0.95 interesting, needs paper
  < 0.80 weak evidence
...
```text
V1:
  research_log ทุก variant
  returns_matrix export
  PSR vs 0 และ empirical SR threshold
  simple CSCV/PBO
...
V2:
  DSR/PBO per regime and strategy family
  survivorship-aware altcoin universe
```

---

## 31. Drift Threshold Calibration

### คำถาม

Research Drift threshold calibration ต่อ

### คำตอบสรุป

Drift threshold ไม่ควรเริ่มจากตัวเลขตายตัว แต่ควรเริ่มจาก:
...
```text
Reference = OOS backtest + paper + early small-live
Monitored = rolling live windows
Threshold = statistical envelope + risk hard limits
```
...
```text
10 trades:
  ห้ามตัดสิน win rate/PF/Sharpe จริงจัง

30 trades:
  เริ่ม watch ได้ แต่ยัง noise สูง

50-100 trades:
  ใช้ warning/critical ได้ดีขึ้น

100+ trades:
  metric มีน้ำหนักขึ้น
```
...
Data Drift:
  ...
  spread > 2x baseline p50 for 5m = Warning
  spread > 3x baseline p50 = Pause entries
...
```text
Drift thresholds must be calibrated from reference distributions and Monte Carlo envelopes,
with minimum sample sizes and multi-signal confirmation.
Hard risk/state violations bypass statistics and trigger immediately.
```

---

## 32. Framework Spike Plan

### คำถาม

Research Framework spike plan แบบปฏิบัติจริงต่อ

### คำตอบสรุป

ทำ spike 4-5 ตัวด้วย strategy/data/metrics เดียวกัน เพื่อเปรียบ framework อย่างยุติธรรม
...
- Spike A: Freqtrade, 2-3 วัน
  - test strategy, backtest, lookahead/recursive analysis, dry-run path
- Spike B: VectorBT, 1-2 วัน
  - parameter sweep, returns_matrix, Monte Carlo, compare with Freqtrade
- Spike C: Custom Mini Engine, 3-5 วัน
  - event models, BacktestBroker, RiskManager, minimal PortfolioLedger
- Spike D: NautilusTrader, 3-5 วัน
  - quickstart, EMA strategy, reports, learning curve
- Optional Spike E: Jesse, 1-2 วัน
  - backtest/Monte Carlo only, check pricing/live limits
...
## 33. Order Type to Signal Urgency Mapping

### คำถาม

Research Order Type -> Signal Urgency Mapping ต่อ

### คำตอบสรุป

Order Type -> Signal Urgency Mapping ควรเป็น policy layer ระหว่าง Risk Manager กับ Broker ไม่ใช่ decision ที่ strategy เลือกเองโดยตรง
...
| Urgency | ตัวอย่าง | Order type |
|---|---|---|
| PASSIVE | grid entry, rebalance ไม่รีบ | post-only limit / limit GTC |
...
```text
Strategy emits urgency, not raw order type.
ExecutionPlanner chooses order type.
Broker capability flags control allowed choices.
Urgent exits prioritize fill certainty.
Passive entries prioritize price/fees.
Unknown capability or stale data = reject or PAUSED.
```

---

## 34. Paper Broker Implementation Details

### คำถาม

Research Paper Broker Implementation Details ต่อ

### คำตอบสรุป

PaperBroker ไม่ควรเป็นแค่ BacktestBroker ที่ใช้ราคาสด แต่ควรเป็น simulation layer ที่ใช้ live market data แล้วจำลอง friction ของ live execution

ต้องใช้ interface เดียวกับ LiveBroker:
...
Post-only buy rejected/canceled if price >= current ask
Post-only sell rejected/canceled if price <= current bid
```
...
```text
PaperBroker is a live deployment rehearsal layer.
It must share BrokerInterface, events, RiskManager, and PortfolioLedger with LiveBroker.
It should be pessimistic enough to reveal execution problems.
```
...
- Bailey & Lopez de Prado DSR / PBO / CSCV papers
- MLFinLab backtest statistics references
- NannyML threshold and drift monitoring references
- Jesse Monte Carlo docs
- Freqtrade lookahead / recursive / backtesting docs
- Bybit / CCXT / exchange order type references
- NautilusTrader sandbox / PaperBroker related references
