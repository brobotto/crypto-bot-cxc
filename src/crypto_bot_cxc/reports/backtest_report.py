...
def write_backtest_report(
    ...
    *,
    ...
    summary = summarize(result=result, initial_cash=initial_cash)
...
def summarize(result: BacktestResult, *, initial_cash: Decimal) -> SummaryMetrics:
...
def _write_summary(path: Path, summary: SummaryMetrics) -> None:
    payload = {key: str(value) for key, value in asdict(summary).items()}
